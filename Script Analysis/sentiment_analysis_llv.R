...
llv <- paste(readLines("./Script Analysis/LLV Script.html"))
...
llv <- data.frame(llv, stringsAsFactors = FALSE) %>%
  filter(!llv %in% c("", "</br>"))
...
llv_sentiment <- tidy_llv %>%
  inner_join(get_sentiments("bing")) %>%
  count(word, sentiment)

write_rds(llv_sentiment, "nicolas_cage_analysis/RDS Files/llv_sentiment.rds")
...
llv_sentiment2 <- tidy_llv %>%
  inner_join(get_sentiments("afinn")) %>%
  arrange(score) %>%
  mutate(average = mean(score))

write_rds(llv_sentiment2, "nicolas_cage_analysis/RDS Files/llv_sentiment2.rds")
...
write_rds(llv_plot, "nicolas_cage_analysis/RDS Files/llv_plot.rds")
